...
export const getAllProjectStatuses = async (): Promise<ProjectStatusListResponseDTO> => {
    try {
        ...
        const data = statuses.map((status) => {
            const { _id, name, status: statusText, statusNumber, isFinal, isPause } = status;
            return { 
                id: _id.toString(), 
                name, 
                status: statusText,
...
export const getProjectStatusById = async (id: string): Promise<ProjectStatusResponseDTO> => {
    try {
        ...
        const { _id, name, status: statusText, statusNumber, isFinal, isPause } = status;
        const data = { id: _id.toString(), name, status: statusText, statusNumber, isFinal, isPause };
        return { isSuccess: true, data };
    } catch (error) {
...
export const createProjectStatus = async (statusData: Partial<ProjectStatusDTO>): Promise<ProjectStatusResponseDTO> => {
    try {
        const status = new ProjectStatus(statusData);
        const savedStatus = await status.save();
        const { _id, name, status: statusText, statusNumber, isFinal, isPause } = savedStatus;
        const data = { id: _id.toString(), name, status: statusText, statusNumber, isFinal, isPause };
        return { isSuccess: true, data };
    } catch (error) {
...
export const updateProjectStatus = async (id: string, updateData: Partial<ProjectStatusDTO>): Promise<ProjectStatusResponseDTO> => {
    try {
        ...
        const { _id, name, status: statusText, statusNumber, isFinal, isPause } = updatedStatus;
        const data = { id: _id.toString(), name, status: statusText, statusNumber, isFinal, isPause };
        return { isSuccess: true, data };
    } catch (error) {
...
export const deleteProjectStatus = async (id: string): Promise<ProjectStatusResponseDTO> => {
    try {
        ...
        const { _id, name, status: statusText, statusNumber, isFinal, isPause } = deletedStatus;
        const data = { id: _id.toString(), name, status: statusText, statusNumber, isFinal, isPause };
        return { isSuccess: true, data };
    } catch (error) {
